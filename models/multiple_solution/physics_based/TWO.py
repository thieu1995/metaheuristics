...
class BaseTWO(RootAlgo):
    ...
    def _update_weight__(self, pop=None):
        ...
        #print(pop[i][self.ID_WEIGHT])    
        return pop

    def _amend_solution_and_return__(self, solution_old=None, solution=None, best_solution=None, epoch=None):
    ...
    def _train__(self):
        ...
        for i in range(self.epoch):
            for j in range(self.pop_size):
                ...
                pop_new[j][self.ID_POS] = self._amend_solution_and_return__(pop[j][self.ID_POS], pop_new[j][self.ID_POS], g_best[self.ID_POS], i+1)
                pop_new[j][self.ID_FIT] = self._fitness_model__(pop_new[j][self.ID_POS], self.ID_MAX_PROBLEM)


            # temp = pop + pop_new
            # temp = sorted(temp, key=lambda x: x[self.ID_FIT], reverse=True)
            # pop = temp[:self.pop_size]
            # pop_new = deepcopy(pop)
