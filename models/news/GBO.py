#!/usr/bin/env python
# ------------------------------------------------------------------------------------------------------%
# Created by "Thieu" at 16:53, 11/01/2022                                                               %
#                                                                                                       %
    ...
    def train(self):
        ...
        for epoch in range(self.epoch):
            ...
            for idx in range(0, self.pop_size):
                ...
                x1 = pop[idx][self.ID_POS] - np.random.normal() * p1 * 2*delta_x*pop[idx][self.ID_POS]/(g_worst[self.ID_POS] - g_best[self.ID_POS]+epsilon) + \
                ...
                if np.random.rand() < self.pr:
                    ...
                    if np.random.rand() < 0.5:
                        pos_new = pos_new + f1 * (u1 * g_best[self.ID_POS] - u2 * x_m) + f2*p1*(u3*(x2 - x1) + u2 * (pop[r1][self.ID_POS]-pop[r2][self.ID_POS]))/2
                    else:
                        pos_new = g_best[self.ID_POS] + f1*(u1*g_best[self.ID_POS] - u2*x_m) + f2*p1*(u3*(x2 - x1) + u2*(pop[r1][self.ID_POS] - pop[r2][self.ID_POS]))/2
